...
| AG6 | [PHASES/AG6-student-ide-harness.md](PHASES/AG6-student-ide-harness.md) | Project-level IDE MCP (Astro+Svelte+Playwright+MCP-org reference), `llms.txt` index, verify script, harness roles in `AGENTS.md` / `agentic/ide-mcp` | **DONE-with-one-open-item** (2026-09-19) — offline + live (real MCP handshake) verification both pass, cold-reviewed with independent reproduction; branch `agentic/ag6-student-ide-harness` pushed, not merged; open item is a human-only GUI walkthrough ([`PHASE-AG6-REPORT.md`](PHASE-AG6-REPORT.md)) |
