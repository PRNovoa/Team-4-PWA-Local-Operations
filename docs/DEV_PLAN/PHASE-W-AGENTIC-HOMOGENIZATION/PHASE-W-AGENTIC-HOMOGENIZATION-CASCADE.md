...
| AG5 | [PHASES/AG5-verify-and-merge-gate.md](PHASES/AG5-verify-and-merge-gate.md) | Full verification + pedagogical merge note + PR checklist; merge only by named human | READY (AG4 DONE) — awaiting explicit authorization; DONE when cold review green **and** product owner records merge decision (merge itself is outside agent authority). May record `AG6_DEFERRED` |
| AG6 | [PHASES/AG6-student-ide-harness.md](PHASES/AG6-student-ide-harness.md) | Project-level IDE MCP (Astro+Svelte), `llms.txt` index, verify script, harness roles in `AGENTS.md` / `agentic/ide-mcp` | BLOCKED on AG4 (content) / preferably after AG5 merge decision; DONE when offline verify is load-bearing and React MCP is accepted-portable or deferred |
