...
| W | Agentic Homogenization — make `agentic/` the tool-neutral edit-home for TTOD rules/skills/agents (extending the existing `report-steward` pack shape); reduce `.cursor/` and `.claude/` to thin **landings** (real frontmatter + redirect). Reconciles the split-brain with studio `~/src/.agents/` via an AG0 naming freeze. Includes a planned branch `agentic/homogenize-landings` (not created by the planning pack) and a pedagogical merge isomorphism to the Phase V quote-proposal accept/merge ritual. **AG6** adds project-level student IDE MCP (Astro docs + official Svelte MCP + vendored `llms.txt`) with existence verification — Docker `services/mcp` stays application-level only. **Does not** mutate `ttod.yml`, migrate all studio skills, ship React Smithery MCP by default, or claim a wired frontier+Ollama validator fleet for AG0–AG5. | gated cascade AG0→AG6; branch only after AG0+AG1 authorization | **AG0–AG5 merged to `main`** via [PR #21](https://github.com/ruvebal/ttod/pull/21) (2026-09-18/19; decision frozen at [`DECISIONS/W0-2026-09-18-AGENTIC-HOME.md`](DECISIONS/W0-2026-09-18-AGENTIC-HOME.md)). **AG6 DONE-with-one-open-item** (2026-09-19): committed IDE MCP config for Astro/Svelte/Playwright/`filesystem`, official-only allowlist mechanically enforced, and a real live MCP handshake against every server (4/4 pass, independently reproduced by cold review) — on branch `agentic/ag6-student-ide-harness`, pushed, not yet PR'd/merged. Open item: a human still needs to run the manual Cursor-GUI walkthrough (deliverable 9b). Next action: open AG6's PR when ready, and run 9b separately. | [`PHASE-W-AGENTIC-HOMOGENIZATION/INDEX.md`](PHASE-W-AGENTIC-HOMOGENIZATION/INDEX.md) · [`PHASE-W-AGENTIC-HOMOGENIZATION-CASCADE.md`](PHASE-W-AGENTIC-HOMOGENIZATION/PHASE-W-AGENTIC-HOMOGENIZATION-CASCADE.md) · [`RATIONALE.md`](PHASE-W-AGENTIC-HOMOGENIZATION/RATIONALE.md) · [`FINDINGS-2026-09-18.md`](PHASE-W-AGENTIC-HOMOGENIZATION/FINDINGS-2026-09-18.md) · [`AGENTIC-HARNESS.md`](PHASE-W-AGENTIC-HOMOGENIZATION/AGENTIC-HARNESS.md) |
