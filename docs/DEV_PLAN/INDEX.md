...
| X | Public Docs: Agentic IDE/MCP Guides — two public pages sourced from Phase W's merged AG6 work: a developer/collaborator guide (connect an IDE to TTOD's MCP servers, verify it's actually wired) and a research/pedagogy guide (why this cascade governs agentic development the way it does, hedged the same way `research/methodology.md` hedges its own claims). Cites TTOD quotes `arch-049`/`arch-050`/`arch-058` by ID (all public, CC-BY-NC-SA-4.0, unforced matches) and Athanor's Law L8 as a governance parallel — no fabricated "Athanor teaching rationale," confirmed none exists. **Does not** authorize a research study, translate to Spanish in this pass (`alt_lang_missing: true`, same accepted pattern as `reviewing-cohort-prs.md`), or restyle the site beyond two pages + one diagram. | single cascade doc, steps X0→X5, branch only after X0 authorization | **PROPOSED (2026-09-19).** Planning pack only — no branch, no page published. Awaiting product-owner go-ahead, including whether X1's local-Ollama (`qwen2.5-coder:32b`) draft pass should run for real. | [`PHASE-X-AGENTIC-DEV-MCP-GUIDES-CASCADE.md`](PHASE-X-AGENTIC-DEV-MCP-GUIDES-CASCADE.md) |
