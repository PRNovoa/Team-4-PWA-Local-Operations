...
| W | Agentic Homogenization — make `agentic/` the tool-neutral edit-home for TTOD rules/skills/agents (extending the existing `report-steward` pack shape); reduce `.cursor/` and `.claude/` to thin **landings** (real frontmatter + redirect). Reconciles the split-brain with studio `~/src/.agents/` via an AG0 naming freeze. Includes a planned branch `agentic/homogenize-landings` (not created by the planning pack) and a pedagogical merge isomorphism to the Phase V quote-proposal accept/merge ritual. **AG6** adds project-level student IDE MCP (Astro docs + official Svelte MCP + vendored `llms.txt`) with existence verification — Docker `services/mcp` stays application-level only. **Does not** mutate `ttod.yml`, migrate all studio skills, ship React Smithery MCP by default, or claim a wired frontier+Ollama validator fleet for AG0–AG5. | gated cascade AG0→AG6; branch only after AG0+AG1 authorization | **AG0–AG4 DONE (2026-09-18)** — decision frozen at [`DECISIONS/W0-2026-09-18-AGENTIC-HOME.md`](DECISIONS/W0-2026-09-18-AGENTIC-HOME.md), cold-reviewed PASS. Branch `agentic/homogenize-landings` created from `main`@`7a738cae`; TTOD-scoped bodies moved byte-identical into `agentic/`; `.cursor` rule/skill hollowed to thin redirect stubs (`.claude` correctly left untouched — cross-repo cascade subagents stay studio-canonical per W0); `AGENTS.md` now carries the edit-home table and discovery map. 219 tests green throughout, all four phases cold-reviewed PASS with zero findings. AG5–AG6 PROPOSED. Live inventory in FINDINGS. Harness roles: [`AGENTIC-HARNESS.md`](PHASE-W-AGENTIC-HOMOGENIZATION/AGENTIC-HARNESS.md). Next action: authorize AG5 (verify + pedagogical merge note + PR prep — merge itself stays a named-human act). | [`PHASE-W-AGENTIC-HOMOGENIZATION/INDEX.md`](PHASE-W-AGENTIC-HOMOGENIZATION/INDEX.md) · [`PHASE-W-AGENTIC-HOMOGENIZATION-CASCADE.md`](PHASE-W-AGENTIC-HOMOGENIZATION/PHASE-W-AGENTIC-HOMOGENIZATION-CASCADE.md) · [`RATIONALE.md`](PHASE-W-AGENTIC-HOMOGENIZATION/RATIONALE.md) · [`FINDINGS-2026-09-18.md`](PHASE-W-AGENTIC-HOMOGENIZATION/FINDINGS-2026-09-18.md) · [`AGENTIC-HARNESS.md`](PHASE-W-AGENTIC-HOMOGENIZATION/AGENTIC-HARNESS.md) |
