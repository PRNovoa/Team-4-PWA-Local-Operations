...
| W | Agentic Homogenization — make `agentic/` the tool-neutral edit-home for TTOD rules/skills/agents (extending the existing `report-steward` pack shape); reduce `.cursor/` and `.claude/` to thin **landings** (real frontmatter + redirect). Reconciles the split-brain with studio `~/src/.agents/` via an AG0 naming freeze. Includes a planned branch `agentic/homogenize-landings` (not created by the planning pack) and a pedagogical merge isomorphism to the Phase V quote-proposal accept/merge ritual. **AG6** adds project-level student IDE MCP (Astro docs + official Svelte MCP + vendored `llms.txt`) with existence verification — Docker `services/mcp` stays application-level only. **Does not** mutate `ttod.yml`, migrate all studio skills, ship React Smithery MCP by default, or claim a wired frontier+Ollama validator fleet for AG0–AG5. | gated cascade AG0→AG6; branch only after AG0+AG1 authorization | **AG0–AG5 DONE (2026-09-18; AG5 DONE-with-MERGE_DEFERRED)** — decision frozen at [`DECISIONS/W0-2026-09-18-AGENTIC-HOME.md`](DECISIONS/W0-2026-09-18-AGENTIC-HOME.md). TTOD-scoped bodies moved into `agentic/`; `.cursor` rule/skill hollowed to landings; `AGENTS.md` carries the edit-home table, discovery map, and pedagogical merge note. **[PR #21](https://github.com/ruvebal/ttod/pull/21) open against `main`** — five independent cold reviews, all PASS, zero blocking findings; not yet merged. AG6 PROPOSED, `AG6_DEFERRED` recorded, does not block merge. **Next action: product owner reviews PR #21 and records the merge decision** — merge is outside any agent's authority regardless of instruction phrasing. | [`PHASE-W-AGENTIC-HOMOGENIZATION/INDEX.md`](PHASE-W-AGENTIC-HOMOGENIZATION/INDEX.md) · [`PHASE-W-AGENTIC-HOMOGENIZATION-CASCADE.md`](PHASE-W-AGENTIC-HOMOGENIZATION/PHASE-W-AGENTIC-HOMOGENIZATION-CASCADE.md) · [`RATIONALE.md`](PHASE-W-AGENTIC-HOMOGENIZATION/RATIONALE.md) · [`FINDINGS-2026-09-18.md`](PHASE-W-AGENTIC-HOMOGENIZATION/FINDINGS-2026-09-18.md) · [`AGENTIC-HARNESS.md`](PHASE-W-AGENTIC-HOMOGENIZATION/AGENTIC-HARNESS.md) |
